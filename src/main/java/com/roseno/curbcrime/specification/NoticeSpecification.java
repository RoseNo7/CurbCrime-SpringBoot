...
public class NoticeSpecification {
    ...
    public static Specification<Notice> bySearchOption(String searchOption, String keyword) {
        return switch (Optional.ofNullable(searchOption).orElse("")) {
            case SEARCH_OPTION_TITLE -> titleContains(Optional.ofNullable(keyword).orElse(""));
            case SEARCH_OPTION_CONTENT -> contentContains(Optional.ofNullable(keyword).orElse(""));
            case SEARCH_OPTION_TITLE_OR_CONTENT -> titleOrContentContains(Optional.ofNullable(keyword).orElse(""));
            default -> titleContains(Optional.ofNullable(keyword).orElse(""));
        };
    }
}
